...
import {
  ...
  Keyboard,
  Mouse,
} from '@dboterho/guacamole-common-js';
...
@Component({
  ...
  template: ` <div class='ngx-remote-desktop-display' #display></div> `,
...
export class DisplayComponent implements OnInit, OnDestroy, AfterViewChecked, AfterViewInit {
   ...
   */
  ngOnInit(): void {
      console.log('Create the display canvas when initialising the component');
  }
  ...
  private bindSubscriptions(): void {
    this.subscriptions.push(
      this.manager.onKeyboardReset.subscribe((_) => this.resetKeyboard())
    );
    this.subscriptions.push(
      this.manager.onFocused.subscribe(this.handleFocused.bind(this))
    );
  }
  ...
  private bindDisplayInputListeners(): void {
    ...
    this.mouse.onmousedown =
      ...
      this.mouse.onmousemove =
        this.handleMouseState.bind(this);
